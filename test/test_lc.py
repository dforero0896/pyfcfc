...
#results = py_compute_cf([np.c_[data, wdat], np.c_[rand, wran]], np.arange(0, 200, 1, dtype=np.double), None, 100, "test/fcfc_lc_ell.conf")
...
results = py_compute_cf([np.c_[data, wdat], np.c_[rand, wran]], np.arange(0, 200, 1, dtype=np.double), np.arange(0, 200, 1, dtype=np.double), 0, "test/fcfc_lc_wp.conf")
...
#results = py_compute_cf([np.c_[data, wdat], np.c_[rand, wran]], np.arange(0, 200, 1, dtype=np.double), None, 0, "test/fcfc_lc_iso.conf")  
