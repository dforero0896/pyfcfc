...
def test_cutsky():
    ...
    nthreads = 256
    fig, ax = pplt.subplots(nrows=1, ncols=3, share=0)#, figsize=(15,5))

    print("Loading randoms...", flush = True)
    randoms_list = [f"{RAND_ROOT}/cutsky_LRG_S{i}_{cap}.fits" for i in range(1000, 3000, 100)][:n_rand_splits]
    ...
    output_base = f"/global/cfs/cdirs/desi/cosmosim/FirstGenMocks/EZmock_post/{file_dir.replace('/global/cfs/cdirs/desi/cosmosim/FirstGenMocks/EZmock/', '')}/{cap}/{zmin:.1f}z{zmax:.1f}f{f:.3f}/"
    ...
    for i in range(n_rand_splits):
        ...
        result += TwoPointCorrelationFunction('smu', edges, data_positions1=data, data_weights1=wdata,
                                ...
                                engine='corrfunc', nthreads=int(os.environ.get("OMP_NUM_THREADS")),
                                position_type = 'rdd',
                                los = 'midpoint',
                                D1D2 = D1D2,
    ...
    for ill, ell in enumerate([2 * i for i in range(3)]):
        ...
        pax[ill].plot(total_results['s'], total_results['multipoles'][ill,:] / corr - 1, label = "pycorr/pyfcfc - 1")
    ax[2].legend(loc='top')
    #pax[0].legend(loc='bottom')
    pax.format(ylim = (-0.1, 0.1))
    fig.tight_layout()
    fig.savefig("test/pyfcfc-v-pycorr.png", dpi=300)
...
def test_paircounts():
    ...
    result = TwoPointCorrelationFunction('smu', edges, data_positions1=[data[:,0], data[:,1], data[:,2]], data_weights1=wdata,
                                data_positions2=None, data_weights2=None,
                                randoms_positions1=[data[:,0], data[:,1], data[:,2]], randoms_weights1=wdata,
                                randoms_positions2=None, randoms_weights2=None,
                                shifted_positions1 = None,
                                shifted_weights1 = None,
                                engine='corrfunc', nthreads=int(os.environ.get("OMP_NUM_THREADS")),
                                position_type = 'xyz',
                                los = 'midpoint',
                                D1D2 = None,
    ...
    results = py_compute_cf([data, data], 
                                ...
                                data_struct = 1,
                                verbose = 'T')
    print(results['pairs']['DD'].shape) #* 
    print(results['normalization']['DD'])
    print(results['pairs']['DD'] * results['normalization']['DD'])
