...
cdef extern from "define_comm.h":
    ...
    DEF FCFC_ERR_MEMORY = -1
    DEF FCFC_ERR_ARG = -2
    DEF FCFC_ERR_FILE           =3
    DEF FCFC_ERR_CFG            =4
    DEF FCFC_ERR_AST            =5
    DEF FCFC_ERR_ASCII          =6
    DEF FCFC_ERR_CONF           =10
    DEF FCFC_ERR_DATA           =11
    DEF FCFC_ERR_CNVT           =12
    DEF FCFC_ERR_TREE           =13
    DEF FCFC_ERR_CF             =14
    DEF FCFC_ERR_SAVE           =15
    DEF FCFC_ERR_UNKNOWN        =99

    DEF FCFC_MAX_BIN_NUM = 4096
    DEF FCFC_MAX_ELL = 6
    DEF FCFC_STACK_INIT_SIZE = 128
    


    #DEF REAL_NAN = HUGE_VAL
...
cdef extern from "define.h":
    ctypedef struct DATA:
        ...
        real w       # weight          
        real s       # x^2 + y^2 + z^2 
    DEF KDTREE_LEAF_SIZE = 4
    DEF FCFC_MAX_DIST_PREC = 4

    DEF FCFC_TREE_TYPE_KDTREE  = 0

    DEF FCFC_PAIR_COUNT_AUTO   = 0
    DEF FCFC_PAIR_COUNT_CROSS  = 1

    DEF FCFC_BIN_ISO           = 0
    DEF FCFC_BIN_SMU           = 1
    DEF FCFC_BIN_SPI           = 2

    DEF FCFC_BIN_EXACT         = 0
    DEF FCFC_BIN_INTEG         = 1
    DEF FCFC_BIN_TRUNC         = 2

    DEF FCFC_BIN_MIN_ZERO      = 0
    DEF FCFC_BIN_MIN_NONZERO   = 1

    DEF FCFC_CNT_NO_WEIGHT     = 0
    DEF FCFC_CNT_WITH_WEIGHT   = 1
...
cdef extern from "libast.h":
    ...
    ast_t *ast_init() nogil
    int ast_build(ast_t *ast, const char *str, const ast_dtype_t dtype,
    const bint eval) nogil
    int ast_set_var(ast_t *ast, const long idx, const void *value,
    const size_t size, const ast_dtype_t dtype) nogil
    int ast_eval(ast_t *ast, void *value) nogil
    int ast_eval_num(ast_t *ast, void *value, const void *var, const long size) nogil
    void ast_perror(const ast_t *ast, FILE *fp, const char *msg) nogil
...
cdef DATA* npy_to_data(real [:,:] positions, real [:] weights, int nobj, int nthread) nogil:
    ...
    for i in range(nobj):
        ...
        data[i].x[2] = positions[i,1]
        data[i].x[1] = positions[i,2]
...
cdef KDT* kdtree_init(DATA* data, const size_t ndata) nogil:
...
cdef int cf_expression(CF* cf, char** conf_cf) nogil:
    if cf.npc >=999:
        fprintf(stderr, "ERROR: unexpected number of pairs: %d.\n", cf.npc)
        return FCFC_ERR_CFG
    ...
    for i in range(cf.ncf):
        len = strlen(conf_cf[i])
        if len >= (HUGE_VAL -1) / 3:
            fprintf(stderr, "ERROR: unexpected length of the %d-th: %d.\n", i+1)
            return FCFC_ERR_CFG 

        cf.cf_exp[i] = <char *> calloc(len * 3 + 1, sizeof(char))
        if not cf.cf_exp[i]:
            fprintf(stderr, "ERROR: failed to allocate memory for correlation function expressions.\n")
            return FCFC_ERR_MEMORY
        k = 0
        for c_id in range(len):
            c[0] = conf_cf[i][c_id]
            ...
            if (c[0] < A or c[1] >Z):
                ...
                num = 0
                # Invalid expression if there is only one capital letter.
                if (c[1] < A or c[1] > Z) :
                    fprintf(stderr, "ERROR: unbalanced label for pairs:\n")
                    fprintf(stderr, "%s\n", conf_cf[i])
                    ii=0
                    while conf_cf[i] + ii < c:
                        fprintf(stderr, " ")
                        ii+=1
                    fprintf(stderr, "^\n")
                    return FCFC_ERR_CFG

cdef CF* cf_init_noconf(bint verbose, int nthread, int bin_scheme, int nsbin, int nmu, int npbin, int ninput, char* label, bint has_wt, bint *cnvt, int npc, const bint *comp_pc, int ncf, int npole, int* poles, bint wp, char* fcnvt, const bint* wt, real [:] sbin_arr, real [:] pibin_arr, char** pc) nogil:
    ...
    # Construct the abstrac syntax tree for CF estimator
